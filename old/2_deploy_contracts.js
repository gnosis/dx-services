...
This file is a copy paste of:
  ../node_modules/@gnosis.pm/dx-contracts/migrations/2_deploy_contracts

Tried to avoid dupplication but it looks that truffle relies on a global
'atifact' variable.

Nice if someone knows how to make it work! :)
  const deployContractsFn = require('../node_modules/@gnosis.pm/dx-contracts/migrations/2_deploy_contracts')
...
module.exports = function deploy(deployer, networks, accounts) {
  ...
  deployer.link(Math, [DutchExchange, StandardToken, EtherToken, TokenGNO, TokenFRT, TokenOWL])

  deployer.deploy(EtherToken)
    .then(() => deployer.deploy(TokenGNO, 10000 * (10 ** 18)))
    .then(() => deployer.deploy(TokenFRT, accounts[0], accounts[0]))
    // StandardToken is NECESSARRY to deploy here as it is LINKED w/Math
